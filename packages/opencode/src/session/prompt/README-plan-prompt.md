...
This file contains the full text of the plan mode system prompt used by opencode.

## Source files

- `plan.txt` – injected as a `<system-reminder>` at the start of every plan-mode turn (OpenCode variant)
- `plan-reminder-anthropic.txt` – richer variant used when the Anthropic provider is active; includes the enhanced planning workflow
